...
function validaToken($id, $token, $con)
{
    ...
    if ($sql->fetchColumn() > 0) {
        if (activarUsuario($id, $con)) {
            ...
            $msg = "Erro al activar cuenta";
        }
    } else {
        $msg = "Error al encontrar datos del cliente";
    }
    return $msg;
}
...
{
    $sql = $con->prepare("SELECT id, usuario, password, nombres FROM admin WHERE usuario LIKE ? AND activo=1 LIMIT 1");
    $sql->execute([$usuario]);
