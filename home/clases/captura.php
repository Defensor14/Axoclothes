...
if (is_array($datos)) {
    ...
    $sql = $con->prepare("SELECT email FROM clientes WHERE id=? AND status=1");
    ...
    if ($id > 0) {
        ...
        if ($productos != null) {
            foreach ($productos as $clave => $cantidad) {
                ...
                $sql = $con->prepare("SELECT id, nombre, price, descuento FROM producto WHERE id=? AND activo=1");
                $sql->execute([$clave]);
                ...
                $sql_insert = $con->prepare("INSERT INTO detalle_compra (id_compra, id_producto, nombre, price, cantidad) VALUES (?,?,?,?,?)");
